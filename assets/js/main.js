...
/* ================= Team Members =================
   This version injects the team cards and uses inline SVG icons (FB/TW/IG)
   so they visually match the original icons' size and weight.
*/
function populateTeamMembers(members = [], assets = {}) {
  try {
    ...
    // Try to locate the wrapper row in your provided markup.
    ...
    // Fallback: create a wrapper that matches your markup if not present
    if (!wrapper) {
    ...
    // Inline SVGs used for social icons: kept small and with class fill-current so they adapt color
    ...
    const html = items.map((entry, idx) => {
      ...
      // Social links from Contentful fields
      ...
      // If the team member provided links, open in new tab.
...
async function loadContentfulIntegration() {
  ...
  try {
    ...
    // homepage hero update (best effort)
    const homepage = items.find(i => i.fields?.slug === 'home')
      || items.find(i => i.fields && (i.fields.heroTitle || i.fields.title || i.fields.headline || i.fields.hero))
      || items[0];
    if (homepage?.fields) {
      const f = homepage.fields;
      const newTitle = getPlainText(f.heroTitle || f.title || f.headline || f.hero || '');
      const newSub = getPlainText(f.subtitle || f.description || f.heroSubtitle || '');
    ...
    // FEATURES
    const features = items.filter(it => String(it?.sys?.contentType?.sys?.id || '').toLowerCase() === 'feature');
    ...
    console.log('✅ Injected Contentful content (features/pricing/faq/testimonials/team/blog).');
  } catch (err) {
